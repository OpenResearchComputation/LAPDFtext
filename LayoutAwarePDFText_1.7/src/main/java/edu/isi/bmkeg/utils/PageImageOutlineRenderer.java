...
public class PageImageOutlineRenderer {
	...
	public static void createPageImage(PageBlock page, String pdfFilename,String outputFileName,int mode) {
		int width = page.getPageBoxWidth();
		int height = page.getPageBoxHeight();
        String pdfID=pdfFilename.substring(pdfFilename.lastIndexOf("/")+1);
		BufferedImage image = new BufferedImage(width, height,
				BufferedImage.TYPE_INT_ARGB);
		image.getGraphics().setColor(Color.white);
		image.getGraphics().fillRect(0, 0, width, height);
		image.getGraphics().setColor(Color.red);
        drawWord(width/2-50, 10, image, Color.black, pdfID+":"+page.getPageNumber());
		List list = page.getAllChunkBlocks(SpatialOrdering.MIXED_MODE);
		// renderBlockPerImage(list, image, fileName);
		renderBlockByColor(list, image,mode);
		
		list = page.getAllWordBlocks(SpatialOrdering.MIXED_MODE);
		renderBlockByColor(list, image,mode);
	...
	private static void renderBlockByColor(List<Block> entityList,
			...
			if (block instanceof ChunkBlock) {
				...
				if (colorCode == TYPE_UNCLASSIFIED_COLOR_CODE) {
					if (countMap.get(Block.TYPE_UNCLASSIFIED) == null) {
						...
						countMap.put(Block.TYPE_UNCLASSIFIED, countMap
								.get(Block.TYPE_UNCLASSIFIED) + 1);
					}
				} else {
					if (countMap.get(chunky.getType()) == null) {
						countMap.put(chunky.getType(), 1);
					} else {
						countMap.put(chunky.getType(), countMap.get(chunky
								.getType()) + 1);
					}
				}}
				drawRectangle(chunky.getX1(), chunky.getY1() , chunky
						.getWidth() , chunky.getHeight() , image,
				...
				Integer colorCode = (mode==1)?colorDecider(chunky.getType()):0x000000;
				
				drawRectangle(wordBlock.getX1()+2, wordBlock.getY1()+2, wordBlock
						.getWidth()-4, wordBlock.getHeight()-4, image, new Color(colorCode),
						block,mode);
	...
	private static void drawRectangle(int x, int y, int width, int height,
		...
		if (block instanceof WordBlock&&mode==1) {
	...
	private static void drawWord(int x, int y,
			BufferedImage image, Color color, String word) {
		Graphics2D graphics = image.createGraphics();
	    Font plainFont = new Font(Font.SANS_SERIF, Font.PLAIN, 10);
